...
cdfs <- list.files(full.names = TRUE)
...
In our specific case, the data frame with the phenotypic data could be designed as follow:
...
These are the retention times in seconds for the chromatography of all loaded files. For example, `r names(rt[1])` stands for File1, scan number `r gsub(".*S", "", names(rt[1]))` and it was recorded at `r round(rt[1])` seconds.
...
As we discussed, metabolites are visible as peaks in 3D rt/mz/intensity plane ...
...
The previous plot is important: it is telling us that the *metabolite_I\_know* is present in the samples and is released by the chromatographic column at around 935 sec ... There it is producing a peak in the signal of the *ion_I\_know* @mz413
...
Also here a plethora of approaches is available. As usual, everything will work better if the chromatography is more reproducible (for GC, for example, retention time correction is often not necessary).
